...
Выполнил: Болтенков Евгений Олегович АВТ-314  

Вариант работы - 78 (Создание функции языка C/C++)  

1.1. Уникальность идентификаторов (правило 1)  
Что проверяется:  
Имя функции не должно быть объявлено ранее в той же области видимости.  
int first(int x) { return x; }  
int first(int y) { return y; }   

Ожидаемое сообщение:  
Идентификатор "first" уже объявлен ранее  

Позиция: строка 2, позиция имени функции.    
1.2. Совместимость типов (правило 2)  
Что проверяется:  

Тип возвращаемого значения соответствует объявленному типу функции.  
Типы операндов в бинарных операциях (+, -, *, /) совместимы.  
Пример ошибки 1 (несовместимость в return):  

int test(float x) { return x; }  
Ожидаемое сообщение:  
Несовместимость типов: ожидается Int, получен Float  

1.3. Допустимые значения (правило 3)
Что проверяется:
Числовое значение литерала не выходит за пределы диапазона своего типа.

Пример ошибки:

text  
int test() { return 9999999999; }  
Ожидаемое сообщение:  
Значение 9999999999 выходит за пределы допустимого диапазона для Int  

1.4. Использование объявленных идентификаторов (правило 4)  
Что проверяется:  
Каждый идентификатор в выражении был объявлен ранее (как параметр функции).  

Пример ошибки:  
int test(int x) { return y; }  
Ожидаемое сообщение:  
Использование необъявленного идентификатора "y"  
...
ReturnType -	TypeNode -	Тип возвращаемого значения  
Parameters -	List<ParameterNode> -	Список параметров  
Body -	ReturnStatementNode -	Тело функции (оператор return) 

Атрибуты для визуализации:    

name — имя функции  
returnType — тип возвращаемого значения  
Пример: int main(int x)  

ParameterNode  
Параметр функции.  
  
Name -	string -	Имя параметра  
Type -	TypeNode -	Тип параметра  
Атрибуты для визуализации:  
...
Пример: int x (где x — параметр)  

TypeNode  
Узел, представляющий тип данных.  

TypeName -	string -	Имя типа ("Int" или "Float")
Атрибуты для визуализации:  
...
IntType — для типа int  

FloatType — для типа float  

Узлы операторов  
ReturnStatementNode  
Оператор возврата значения из функции.  

ReturnValue -	ExpressionNode -	Возвращаемое выражение  
Атрибуты для визуализации:  

hasValue — есть ли возвращаемое значение (true/false)  

Пример: return x + 5;  

Узлы выражений  
ExpressionNode (абстрактный)  
Базовый класс для всех выражений.  

string -  GetExpressionType()	Возвращает тип выражения ("Int", "Float", "Unknown")  
BinaryExpressionNode  
Бинарная операция (с двумя операндами).  

Left -	ExpressionNode -	Левый операнд  
Operator	string -	Оператор ("+", "-", "*", "/")  
Right -	ExpressionNode -	Правый операнд  
Атрибуты для визуализации:  

operator — строковое представление оператора  

Пример: a + b (где a — левый операнд, + — оператор, b — правый операнд)  

LiteralNode  
Литерал (числовое значение).  

Value -	object -	Числовое значение  
LiteralType -	string -	Тип литерала ("Int" или "Float")  
Атрибуты для визуализации:  
...
IntLiteral — для целочисленных литералов (например, 42)  

FloatLiteral — для вещественных литералов (например, 3.14)  

IdentifierNode  
Идентификатор (имя переменной/параметра).  
...
<img width="1041" height="598" alt="image" src="https://github.com/user-attachments/assets/3f83f59a-7adc-4026-8fe6-011b6498dfee" />  

<img width="480" height="220" alt="image" src="https://github.com/user-attachments/assets/4d152630-5d1d-4b8e-96b6-b53e6753b6d9" />    

<img width="1482" height="991" alt="image" src="https://github.com/user-attachments/assets/6638d9b4-9fa7-4225-bb72-3359ad4308d4" />  
